...
class EtlSpec extends AnyFreeSpec with Matchers {
  ...
  "etl" - {
    ...
    "outputs an extract error if the input file path doesn't exist" in {
    ...
    "outputs a load error if the output file path doesn't exist" in {
      val inputFilePath = "src/test/resources/testInput2.txt"
      val erroneousOutputFilePath = ""
      etl(inputFilePath, erroneousOutputFilePath)(using Etl.IntImpl) shouldEqual Left(LoadError)
    }
  }
